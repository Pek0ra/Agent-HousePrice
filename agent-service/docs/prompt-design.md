...
- Schema 只包含三个白名单视图，不包含整库结构，减少越权和字段幻觉。
...
1. 模型只能看到 `v_agent_house_listing`、`v_agent_district_summary` 和 `v_agent_monthly_price_trend` 三个清洗视图。
2. SQLGlot 以 MySQL 方言解析 AST，只接受单条查询语句，并从 AST 检查表白名单。
3. 拒绝写操作、DDL、无控制的 `SELECT *`、多语句、跨库限定名、SQL 注释、文件导出、锁定读取和常见延时函数。
4. 缺失 `LIMIT` 时自动增加 `LIMIT 100`，过大的 LIMIT 会改写为 100；数据库会话另设 5 秒执行超时。
5. 查询使用 `house_agent_ro`，只对三个视图拥有 `SELECT`；独立的 `house_agent_audit` 只对审计表拥有 `INSERT`。
6. `deleted=0` 和聚合口径固化在数据库视图中，不依赖模型记住逻辑删除条件。
