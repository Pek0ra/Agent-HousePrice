...
SELECT a.* FROM house_info_analysis a JOIN house_active_dataset d ON a.dataset_id=d.dataset_id;

CREATE OR REPLACE VIEW v_agent_house_data_quality_summary AS
SELECT q.* FROM house_data_quality_summary q JOIN house_active_dataset d ON q.dataset_id=d.dataset_id;
